...
public class MailUtil {
    ...
    if (contentType != null) {
      ...
      if (charsetStartIndex >= 0) {
        ...
        if (charsetEndIndex > charsetStartIndex) {
          charset =
              contentType
                  .substring(charsetStartIndex + 8, charsetEndIndex)
                  .replaceAll("=* *\"", "");
        } else {
  ...
  public static String getBoundary(String contentType) {
    ...
    if (boundaryStartIndex >= 0) {
      ...
      if (boundaryEndIndex > 0) {
        boundary =
            contentType.substring(boundaryStartIndex + 9, boundaryEndIndex).replaceAll("\"", "");
      } else {
    ...
    try {
      ...
      if (charset == null) {
        decoded = encoded;
          ...
          || "7bit".equals(contentTransferEncoding.toLowerCase())) {
  ...
  public static String getFileName(String contentDisposition) {
    ...
    if (contentDisposition.indexOf("\"\r\n", dispoName + 10) > dispoName + 10) {
      return MailUtil.decode64(
          contentDisposition.substring(
              dispoName + 10, contentDisposition.indexOf("\"\r\n", dispoName + 10)));
    }
    return "";
  }
}
