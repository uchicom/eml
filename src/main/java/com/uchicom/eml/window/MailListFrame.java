...
public class MailListFrame extends ResumeFrame {
  ...
  public MailListFrame(String[] args) {
  ...
  private List<Account> createAccountList() {
    ...
    if (accountKyes != null) {
      for (String key : accountKyes.split(",")) {
        ...
        Account account =
            new Account(
                config.getProperty(accountKey + Constants.PROP_NAME),
  ...
  private ServerInfo createServerInfo(String key) {
    ...
    System.out.println(serverKey + Constants.PROP_PORT);
    return new ServerInfo(
        config.getProperty(serverKey + Constants.PROP_NAME),
